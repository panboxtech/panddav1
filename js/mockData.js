/* mockData.js
   Contém gerador de dados mock para uso no protótipo.
   Substituição por Supabase:
     - Trocar os métodos que retornam Promises (getClients, getServers, ...).
     - Comentários indicam onde fazer fetch para Supabase (ex.: supabase.from('clients').select()).
*/
...
function seedMockData() {
  ...
  // Planos
  MockDB.plans = [
    { id: uid('pl_'), nome: 'Básico', telas: 2, validadeEmMeses: 1, preco: 29.90 },
    { id: uid('pl_'), nome: 'Pro', telas: 4, validadeEmMeses: 3, preco: 69.90 },
    { id: uid('pl_'), nome: 'Premium', telas: 8, validadeEmMeses: 12, preco: 199.90 },
  ];
  ...
  MockDB.clients = [
    { id: uid('cli_'), nome: 'João Silva', telefone: '99999-0001', email: 'joao@mail.com', dataVencimento: addDays(today, 2).toISOString(), notificado: false, planoId: MockDB.plans[0].id, telas: 2, preco: MockDB.plans[0].preco, pontosAcesso: [] },
    { id: uid('cli_'), nome: 'Maria Souza', telefone: '99999-0002', email: 'maria@mail.com', dataVencimento: addDays(today, -10).toISOString(), notificado: true, planoId: MockDB.plans[1].id, telas: 4, preco: MockDB.plans[1].preco, pontosAcesso: [] },
...
const MockAPI = {
  ...
  createClient(client) {
    return new Promise(resolve => { client.id = uid('cli_'); MockDB.clients.push(client); setTimeout(()=>resolve(client),200); });
    // Supabase: supabase.from('clients').insert([client])
  },
  updateClient(id, patch) {
    return new Promise(resolve => {
      const idx = MockDB.clients.findIndex(c=>c.id===id);
      if(idx>=0){ MockDB.clients[idx] = Object.assign({}, MockDB.clients[idx], patch); setTimeout(()=>resolve(MockDB.clients[idx]),200); }
  ...
  createPlan(p){ p.id = uid('pl_'); MockDB.plans.push(p); return Promise.resolve(p); },
};
